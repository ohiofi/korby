...
class CameraObject extends GameObject {
  constructor() {
    ...
    this.distanceBehindPlayer = 200;
    this.distanceAbovePlayer = 150;
    this.lookAtY = -50;
  }
  ...
  update() {
    this.cam.setPosition(
      player.transform.position.x -
        this.distanceBehindPlayer * sin(player.transform.rotation.y),
      player.transform.position.y - this.distanceAbovePlayer,
      player.transform.position.z -
        this.distanceBehindPlayer * cos(player.transform.rotation.y)
    );
    this.cam.lookAt(
      player.transform.position.x +
        this.distanceBehindPlayer * sin(player.transform.rotation.y),
      this.lookAtY -
        player.transform.rotation.x * 10,
      player.transform.position.z +
        this.distanceBehindPlayer * cos(player.transform.rotation.y)
    );
  }
